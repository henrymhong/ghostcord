...
const ChatViewComponent = ({ chat, avatars, email, username }) => {
  ...
  //       if (bubbleColors[user] === undefined) {
  //         setBubbleColors(prevBubbleColors => {
  //           return {
  //             ...prevBubbleColors,
  //             [user]: "#000000".replace(/0/g, () => {
    ...
    return (
      ...
      >
        ...
        >
          {chat.messages.length === 0 ? (
            ...
            chat.messages.map((msg, index) => {
              // Map all messages
              var bubbleColor = msg.sender !== email ? "#CCCCCC" : "#147efb"; // If user sent message, set color
              var fontColor = msg.sender !== email ? "#000000" : "#FFFFFF"; // If user sent message, set color
              return (
                // Chat bubble
                <div
                  key={index}
        ...
        >
          ...
          />
          <IconButton
            onClick={() => sendMessage(message, chat, email, username)}
          >
            <SendRoundedIcon />
          </IconButton>
        </div>
